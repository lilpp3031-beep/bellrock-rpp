...
import { ScrapedContent, ExtractedMetadata, ProductMetadata } from '../types';
...
export class ExtractorService {
  ...
  async analyzeContent(
    ...
    logger.info(
      { url: scrapedContent.url, imageCount: scrapedContent.images.length, tone: context?.tone, hasAmazonReference: !!context?.amazonReferenceText },
      'Analyzing content with Claude API'
    );
...
- 上記のトーンに合わせた説明`;
  ...
  private buildExtractionPrompt(
    scrapedContent: ScrapedContent,
    context?: { brand?: string; category?: string; tone?: string; amazonReferenceText?: string }
  ): string {
...
- 上記のトーン&マナーに従って説明を生成してください。`;
   ...
   */
  private parseExtractionResponse(responseText: string): ExtractedMetadata {
    try {
   ...
   */
  private fallbackExtraction(scrapedContent: ScrapedContent): ExtractedMetadata {
